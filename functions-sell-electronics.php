...
if(isset($_POST['save'])){
	...
	if(move_uploaded_file($_FILES['image']['tmp_name'], $target_file)){
		...
		$sql="INSERT INTO sell SET image='$image', email='$email', name='$name', description='$description', price='$price', contact='$contact', owner='$owner', c_id='1' ";
	

	// if the data has been saved in the database , redirect the user to the main page

		if(mysqli_query($conn, $sql)){
			header('location:sell-list-electronics.php');
		}
	}
}
...
$results=mysqli_query($conn, "SELECT * FROM sell WHERE c_id = '1'");
